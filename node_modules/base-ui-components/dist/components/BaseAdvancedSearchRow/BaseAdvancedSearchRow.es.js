import c from "../BaseSearch/BaseSearch.es.js";
import d from "../BaseIcon/BaseIcon.es.js";
import p from "../BaseButton/BaseButton.es.js";
import u from "../BaseChipsInputField/BaseChipsInputField.es.js";
import f from "../BaseChip/BaseChip.es.js";
import h from "../BaseDropDownList/BaseDropDownList.es.js";
import { createId as y, sort as m, hasData as _ } from "../../utils/utils.es.js";
import b from "../../mixins/navigateList.es.js";
import v from "../../mixins/i18n.es.js";
import "./BaseAdvancedSearchRow.vue_vue_type_style_index_0_scoped_2ddf8614_lang.es.js";
import "./BaseAdvancedSearchRow.vue_vue_type_style_index_1_lang.es.js";
import g from "../../_virtual/_plugin-vue2_normalizer.es.js";
const w = {
  ...
  computed: {
    ...
    displayedOptions() {
      if (this.controlledVocabularyOptions) {
        ...
        return this.filter.filter_values && this.filter.filter_values.length && (t = t.filter((e) => !this.filter.filter_values.map((r) => r[this.identifierPropertyName.controlledVocabularyOption]).includes(e[this.identifierPropertyName.controlledVocabularyOption]))), this.currentInput && (t = t.filter((e) => e[this.labelPropertyName.filter].toLowerCase().includes(this.currentInput.toLowerCase()))), m(
          t,
    ...
    filterHasValues() {
      return this.filter[this.identifierPropertyName.filter] !== this.defaultFilter[this.identifierPropertyName.filter] || _(this.filter.filter_values);
    },
    searchType() {
      if (this.filter) {
        const { type: t } = this.filter;
  ...
  watch: {
    ...
    appliedFilter: {
      handler(t) {
        if (JSON.stringify(t) !== JSON.stringify(this.filter)) {
          ...
          }, t && t.filter_values ? t.type.includes("date") ? this.currentInput = t.filter_values : t.type === "text" ? [this.currentInput] = t.filter_values : this.currentInput = "" : this.currentInput = "";
        }
      },
      immediate: !0
    },
    ...
    isActive(t) {
      t && (this.filterFade = {
        left: !1,
        right: !0
      }), !t && this.filter.type === "chips" && this.currentInput && this.currentInput.trim() ? this.filter.freetext_allowed && (!this.selectedOptions || !this.selectedOptions.some((e) => !e[this.identifierPropertyName.autocompleteOption] && e[this.labelPropertyName.autocompleteOption] === this.currentInput)) ? this.addOption({ [this.labelPropertyName.autocompleteOption]: this.currentInput }) : this.resetAllInput() : !t && this.filter.type === "text" && this.currentInput && this.filter.filter_values.join() !== this.currentInput && this.addOption(this.currentInput), this.$emit("is-active", t);
    }
  },
  mounted() {
  ...
  methods: {
    ...
    removeFilter() {
      ...
      }, this.resetAllInput()) : this.$emit("remove-filter", this.filter);
    },
    selectFilter(t) {
    ...
    setCollection(t) {
      this.activeCollection = this.activeEntry ? t : "";
    },
    addOption(t, e = "") {
      const r = this.activeCollection || e || this.defaultFilter[this.identifierPropertyName.filter];
      if (this.useAutocompleteFunctionality && t[this.identifierPropertyName.autocompleteOption] && this.filter[this.identifierPropertyName.filter] !== r && !(this.filter.subsets && this.filter.subsets.length && this.filter.subsets.includes(r))) {
        const i = this.filterList.find((l) => l[this.identifierPropertyName.filter] === (this.activeCollection || e)) || this.defaultFilter, s = i[this.identifierPropertyName.filter] === this.defaultFilter[this.identifierPropertyName.filter] ? [].concat(t[this.labelPropertyName.autocompleteOption]) : [].concat(t), a = this.filter.filter_values && this.filter.filter_values.length === 1 && typeof this.filter.filter_values[0] == "string" ? [{
          [this.labelPropertyName.autocompleteOption]: this.filter.filter_values[0]
        }] : [];
        this.filter = {
          ...i,
          filter_values: a.concat(s)
        };
      } else
    ...
    openDropDown() {
      this.isActive = !0, this.searchInputElement && this.searchInputElement.focus();
    },
    navigateDropDown(t) {
      t.preventDefault();
    ...
    navigateAutocomplete(t, e) {
      if (t.preventDefault(), this.resultListInt.length) {
        ...
        if (r === "ArrowDown" || r === "ArrowUp") {
          this.activeCollection || (this.activeCollection = this.resultListInt[0][this.autocompletePropertyNames.id]);
          const i = this.resultListInt.map((o) => o[this.autocompletePropertyNames.id]).indexOf(this.activeCollection);
          let s = this.consolidatedResultList[this.activeCollection];
          const a = e ? 1 : -1, l = s.indexOf(this.activeEntry), n = this.isWithinArrayLimit(
            this.resultListInt,
            e,
            i + a
          );
          ...
          else if (!this.collectionSelect && !e && i === 0 && l === 0)
            this.activeEntry = null;
          else if (n) {
            this.activeCollection = this.resultListInt[i + a][this.autocompletePropertyNames.id], s = this.consolidatedResultList[this.activeCollection];
            const o = e || this.collectionSelect ? 0 : s.length - 1;
            this.activeEntry = s[o];
          } else
            !n && i === this.resultListInt.length - 1 && l === s.length - 1 ? (this.activeCollection = this.resultListInt[0][this.autocompletePropertyNames.id], s = this.consolidatedResultList[this.activeCollection], [this.activeEntry] = s) : !n && i === 0 && l === -1 && (this.activeCollection = this.resultListInt[this.resultListInt.length - 1][this.autocompletePropertyNames.id], s = this.consolidatedResultList[this.activeCollection], [this.activeEntry] = s.slice(-1));
        } else
          r === "ArrowLeft" ? this.collectionSelect = !0 : r === "ArrowRight" && (this.collectionSelect = !1);
      }
    },
    handleDropDownOnTabKey(t) {
      const e = this.$el.getElementsByTagName("input");
      if (e) {
        const r = Array.from(e), i = r.indexOf(t.target);
    ...
    setFilterValues(t, e = {}) {
      const r = e.filter_values, { type: i } = t, s = t.freetext_allowed;
      return i === "date" ? e.type && e.type.includes("date") && r ? r.date_from || r.date_to : "" : i === "daterange" ? e.type.includes("date") ? {
        ...
        date_from: "",
        date_to: ""
      } : i === "chips" && s && e.type === "chips" && e.freetext_allowed ? t.subsets && t.subsets.includes(e[this.identifierPropertyName.filter]) ? r : r && r.length ? r.filter((a) => !a[this.identifierPropertyName.autocompleteOption]) : [] : e.type === "text" && r && r.length && !!r[0] && i === "chips" && s ? [{
        [this.labelPropertyName.autocompleteOption]: r[0]
      }] : [];
    },
    resetAllInput() {
      this.currentInput = "", this.activeEntry = null, this.activeCollection = "", this.activeControlledVocabularyEntry = null;
    },
    calcFadeOut(t) {
...
var O = function() {
  ...
  return r("div", { ref: "advancedSearchRow", staticClass: "base-advanced-search-row", on: { click: e.openDropDown } }, [r("BaseSearch", e._b({ ref: "baseSearch", class: [
    ...
    if (!i.type.indexOf("key") && e._k(i.keyCode, "esc", 27, i.key, ["Esc", "Escape"]))
      return null;
    e.isActive = !1;
  }], "value-validated": e.handleDateInput }, scopedSlots: e._u([{ key: e.filterSlotName, fn: function() {
    return [r("BaseChipsInputField", { class: [
    ...
    return [!e.isMainSearch || e.filterHasValues ? r("button", { class: [
    ...
    } } }, [r("BaseIcon", { staticClass: "base-advanced-search-row__search-row-icon", attrs: { title: e.getI18nTerm(e.getLangLabel(e.advancedSearchText.removeFilter)), name: "remove" } })], 1) : e._e()];
    ...
    return [e.isActive ? r("BaseDropDownList", { ref: "dropDown", staticClass: "base-advanced-search-row__drop-down-body", attrs: { "drop-down-options": e.resultListInt, "active-option": { [e.autocompletePropertyNames.id]: e.activeCollection }, "list-id": "autocomplete-options-" + e.internalRowId, "active-styled": !1, "has-sub-options": !0, "display-as-drop-down": !1, "use-custom-option-active-background-color": !0, language: e.language, "identifier-property-name": e.autocompletePropertyNames.id, "label-property-name": e.autocompletePropertyNames.data, "list-body-style": {
      ...
      i.stopPropagation();
    } }, scopedSlots: e._u([{ key: "before-list", fn: function() {
      return [r("div", { class: [
      ...
      ] }, [r("ul", { ref: "filterBox", staticClass: "base-advanced-search-row__filter-list", attrs: { id: "filter-options-" + e.internalRowId, role: "listbox" } }, e._l(e.displayFilterList, function(i, s) {
        return r("li", { key: s, ref: "filterOption", refInFor: !0, staticClass: "base-advanced-search-row__filter base-advanced-search-row__column-item", class: [
          {
        ...
        ], attrs: { id: `filter-option-${i[e.identifierPropertyName.filter]}`, "aria-selected": (e.filter && e.filter[e.identifierPropertyName.filter] === i[e.identifierPropertyName.filter]).toString(), tabindex: "-1", role: "option" }, on: { keydown: function(a) {
          return !a.type.indexOf("key") && e._k(a.keyCode, "enter", 13, a.key, "Enter") ? null : (a.stopPropagation(), e.selectFilter(i));
        }, click: function(a) {
          return a.stopPropagation(), e.selectFilter(i);
        } } }, [e._v(" " + e._s(`#${i[e.labelPropertyName.filter]}`) + " ")]);
      }), 0)])])])];
    }, proxy: !0 }, { key: "option", fn: function(i) {
      return [!e.filter || e.useAutocompleteFunctionality ? r("div", { staticClass: "base-advanced-search-row__autocomplete-body" }, [i.option[e.autocompletePropertyNames.data].length ? r("div", { class: [
        "base-advanced-search-row__first-column",
        "base-advanced-search-row__autocomplete-collection"
      ] }, [r("div", { staticClass: "base-advanced-search-row__autocomplete-collection-text" }, [e._v(" " + e._s(i.option[e.autocompletePropertyNames.label]) + " ")])]) : e._e(), r("BaseDropDownList", { staticClass: "base-advanced-search-row__autocomplete-options", attrs: { "drop-down-options": i.option[e.autocompletePropertyNames.data], "active-option": e.activeEntry, "display-as-drop-down": !1, "list-id": "autocomplete-options-" + e.internalRowId, language: e.language, "identifier-property-name": e.identifierPropertyName.autocompleteOption, "label-property-name": e.labelPropertyName.autocompleteOption }, on: { "update:activeOption": function(s) {
        e.activeEntry = s;
      }, "update:active-option": [function(s) {
        e.activeEntry = s;
      }, function(s) {
        return e.setCollection(i.option[e.autocompletePropertyNames.id]);
      }], "update:selected-option": function(s) {
        return e.addOption(s, i.option[e.autocompletePropertyNames.id]);
      } } })], 1) : e._e()];
      ...
      ] }, [r("div", { staticClass: "base-advanced-search-row__controlled-options-title base-advanced-search-row__first-column" }, [e._v(" " + e._s(e.getI18nTerm(e.getLangLabel(e.advancedSearchText.availableOptions))) + " ")]), e.controlledVocabularyOptions && e.displayedOptions.length && (e.displayedOptions.length <= e.maxNumberControlledOptions || e.currentInput && e.currentInput.length >= 4) ? r("ul", { staticClass: "base-advanced-search-row__chips-list base-advanced-search-row__columns", attrs: { role: "listbox" } }, e._l(e.displayedOptions, function(i) {
        return r("li", { key: i[e.identifierPropertyName.controlledVocabularyOption], staticClass: "base-advanced-search-row__column-item", attrs: { value: i[e.labelPropertyName.controlledVocabularyOption], "aria-selected": (e.activeControlledVocabularyEntry && i[e.identifierPropertyName.controlledVocabularyOption] === e.activeControlledVocabularyEntry[e.identifierPropertyName.controlledVocabularyOption] || !1).toString(), role: "option", tabindex: "0" }, on: { mouseenter: function(s) {
          ...
          e.activeControlledVocabularyEntry = null;
        } } }, [r("BaseChip", { staticClass: "base-advanced-search-row__option-chip", attrs: { "is-removable": !1, entry: e.getLangLabel(i[e.labelPropertyName.controlledVocabularyOption]), "chip-active": e.activeControlledVocabularyEntry && i[e.identifierPropertyName.controlledVocabularyOption] === e.activeControlledVocabularyEntry[e.identifierPropertyName.controlledVocabularyOption] }, on: { clicked: function(s) {
    ...
    e.currentInput = i;
  }, expression: "currentInput" } }, "BaseSearch", e.$listeners, !1)), e.isMainSearch ? r("BaseButton", { class: [
    ...
    i.stopPropagation();
  } }, scopedSlots: e._u([{ key: "text", fn: function() {
    return [r("span", { staticClass: "base-advanced-search-row__add-filter-button__text" }, [e._v(" " + e._s(e.getI18nTerm(e.getLangLabel(e.advancedSearchText.addFilter))) + " ")])];
  }, proxy: !0 }], null, !1, 2574446168) }) : e._e()], 1);
}, I = [], N = /* @__PURE__ */ g(
  w,
  O,
  I,
...
//# sourceMappingURL=BaseAdvancedSearchRow.es.js.map
